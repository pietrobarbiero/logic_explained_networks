...
def get_reduced_model(model: torch.nn.Module, x_sample: torch.Tensor, device: torch.device = torch.device('cpu')):
    """
    ...
    linear = torch.nn.Linear(weights_reduced.shape[1], weights_reduced.shape[0], bias=False)
