...
def buildtree(rows, split_points, scoref=entropy, class_dominance=98, min_set_size=1, max_depth=30, root=False):
    ...
    if (best_criteria and best_sets and len(best_sets[0]) > min_set_size and len(best_sets[1]) > min_set_size) or root:
        trueBranch = buildtree(best_sets[0], split_points, scoref, class_dominance, min_set_size, max_depth - 1,
                               root=False)
        falseBranch = buildtree(best_sets[1], split_points, scoref, class_dominance, min_set_size, max_depth - 1,
                                root=False)
...
def column_wise_split(split_points, col, rows, current_score, scoref):
    best_gain = -1
    best_criteria = None
    best_sets = None
    column_values = {}
    ...
    for value in column_values.keys():
        ...
        if gain >= best_gain and len(set1) > 0 and len(set2) > 0:
            ...
            best_sets = (set1, set2)
    return best_sets, best_gain, best_criteria
